...
col1 <- c(
  ...
  '$t$',
  '$a$',
  '$\\mathrm{RV}_{a,F}$, $\\mathrm{RV}_{a,M}$',
  '$M_a$',
  '$v_{a,f}$', 
  '$F_{t,f}$',
  '$n_\\mathrm{base}$',
...
col2 <- c(
  "Operating model (data generating model)",
  "Estimation model (assessment model)",
  "Half-sibling pair. We use $\\mathrm{HSP}^=$ to denote HSPs sharing mitochondrial DNA and $\\mathrm{HSP}^{\\neq}$ to 
  denote the ones that do not", 
  "Parent-offspring pair. We use $\\mathrm{POP}^=$ to denote POPs sharing mitochondrial DNA and $\\mathrm{POP}^{\\neq}$ to 
  denote the ones that do not",
...
the offspring, if mtDNA matches. $H_\\mathrm{mt}^\\mathrm{OM}$ and $H_\\mathrm{mt}^\\mathrm{EM}$ are used to denote the $H_\\mathrm{mt}$
simulated in the OM and the $H_\\mathrm{mt}$ assumed in the EM, respectively',
  "Time in years",
  "Age in years",
  'Female and male, respectively, reproductive value (RV) at age $a$. The superscripts OM and EM were used to 
  distinguish between RV simulated in the OM and RV specified or estimated in the EM 
  (e.g., $\\mathrm{RV}_{a,F}^\\mathrm{OM}$, $\\mathrm{RV}_{a,M}^\\mathrm{EM}$). For the females, 
  $\\mathrm{RV}^\\mathrm{OM}$ was always equal to $\\mathrm{RV}^\\mathrm{EM}$. $\\mathrm{RV}_{a,M}^\\mathrm{OM}$
  differed from $\\mathrm{RV}_{a,F}^\\mathrm{OM}$ in some scenarios',
...
tableA1 <- data.frame(Notation = colA1, 
                     Definition = colA2 
) 
