...
the two metrics (Figures S9 and S10).
...
(Figures S11 and S12). Population was included as
...
estimation option are shown in Figure S13 and highlight the similarities between
...
Figures S11 and S12 highlight the effect of stochasticity due to genetic
...
which roughly doubled the number of males sampled (Figure S14 Panel A) and
...
were sampled, but the other kinship types were fairly unaffected (Figure S14 Panel B).
For estimation performance, we found that varying the number of males sampled had no effect 
(Figure S15), further emphasizing the minimal impact of
...
tS1_cap <- "Median number of adult males and females sampled and number of observed related pairs found by OM and EM 
scenario as well as the number of CKMR
sampling years (Yrs) and CKMR sample size (N; proportion of base sample size, see main text for details). The 
number in parentheses is the interquartile range. sHmt = simulated mitochondrial haplotype diversity,
eHmt = mitochondrial haplotype diversity assumed in the EM. In the Base OM, there was no skipped spawning
...
tab_tex <- knitr::kable(
  ...
  add_header_above(c(" " = 4, "Number Sampled" = 2, "CKMR Pairs Found" = 5)) %>%
  kable_styling(
...
\newpage
\begin{landscape}
```{r parnll-plot, echo=FALSE, fig.align='center', fig.height=6.5, fig.width=9, fig.cap = parnll_plot_fig_caption, message=FALSE, warning=FALSE}
para_res_plot_nll 
```

\newpage

```{r par-plot, echo=FALSE, fig.align='center', fig.height=6.5, fig.width=9, fig.cap = par_plot_fig_caption, message=FALSE, warning=FALSE}
para_res_plot 
```
\end{landscape}
\newpage

```{r cor-plot, echo=FALSE, fig.align='center', fig.height=5, fig.width=6.5, fig.cap = cor_plot_fig_caption, message=FALSE, warning=FALSE}
cor_plot 
```
