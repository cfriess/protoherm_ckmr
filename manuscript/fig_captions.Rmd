...
\vspace{1em} 
**Figure 4.** Number of CKMR pairwise relationships by OM configuration. 
...
DNA. In the base OM, there is no skipped spawning, male and female reproductive value (RV) are equal. In this figure and all other boxplot figures in this manuscript, the box represents the interquartile range (IQR), the line is the median, 
...
summarized across years by scenario and by whether or not sex transition 
...
and female RV are equal. When $\Hmte \neq \Hmts$, one was set to 1 and the other to 0.9. When $P^{\mathrm{F}\rightarrow \mathrm{M}}$ 
was fixed, it was fixed at the true value. When $\mathrm{RV}_{M}$ was fixed, however, it was fixed at the same
value as that of females. Consequently when $\mathrm{RV}_M$ is fixed (red and blue boxes), it is misspecified in the EM when
$\mathrm{RV}_M$ was simulated to differ from $\mathrm{RV}_F$ in the OM (panels E and F). Y-axis truncated to improve readability. 
Notice the different y-axis scale for panel C. Boxplot elements are as described for Figure 4.
...
for all scenarios and by whether or not sex transition 
($P^{\mathrm{F}\rightarrow \mathrm{M}}$) and male reproductive value 
($\mathrm{RV}_{M}$) functions are assumed known (fixed). In the Base scenario there is no skipped spawning, male and female RV are
equal, and $\Hmte = \Hmts$ = 1. When $\Hmte \neq \Hmts$, one was set to 1 and the other to 0.9.
Notice the different y-axis scale for panel B. Y-axis truncated to improve readability. Boxplot elements are as described for Figure 4.

\vspace{1em} 
**Figure 7.** Estimated sex transition and male reproductive value ($RV_M$) functions by scenario.
The dashed black lines are the simulated values and solid black lines are the median estimated values. $\Hmts$ = simulated mitochondrial haplotype diversity, $\Hmte$ = mitochondrial haplotype diversity assumed in the estimation model. In the Base scenario there is 
no skipped spawning, male and female RV are
equal, and $\Hmte = \Hmts$ = 1. When $\Hmte \neq \Hmts$, one was set to 1 and the other to 0.9. Panel 
lettering refers to each row of plots: a) Base; 
