...
abstract: |
  ...
  for protogynous fishes and include them in an integrated stock assessment framework
  for estimating abundance. Using simulation, we evaluate the robustness of this approach to 
  uncertainty in male contribution to reproductive success and CKMR data information content and structure. 
  We model our simulation after Gulf of Mexico Gag whose population has experienced large 
...
We start out this section describing the model species (Section 2.2), 
followed by an overview of the CKMR kinship probabilities for protogynous hermaphrodites (Section 2.3).
We then present the different components of the simulation study to test this model.
We use an individual-based operating model (OM; Section 2.4) to generate pedigrees for a population 
of protogynous hermaphrodites. Each simulation creates millions of
fish _in silico_ with known birth and death dates, sex, and mortality causes, and a known pedigree 
that connects them all. The simulated population experiences increasing fishing 
mortality, resulting in population decline and growing sex-ratio skew, consistent with trends observed in the 
real-world Gag population. In the terminal years of the simulation, we sample individuals from the OM using data collection 
strategies that vary both the number of sampling years and sample size (Section 2.5). We generate both 
traditional fisheries data (i.e., a relative abundance index and composition data) and
CKMR kin pairs. The simulated 'observed' data serve as inputs in a simple, integrated fisheries stock-assessment-style estimation 
model (EM; Section 2.6). The EM uses the 'observed' data and the derived CKMR kinship probabilities to estimate a set of parameters 
that allow generating 'estimated' abundance which can then be compared to the 'true' abundance 
generated by the OM. Finally, we quantify and explore the bias in estimated abundance and male sex ratio 
under different scenarios (Section 2.6). The first group of scenarios explores the effect of CKMR data information 
content and structure and the second group addresses reproductive process uncertainty focused on the male contribution 
to reproductive success.

## 2.2. Model Species

We parameterized our simulation model based on life history data for Gag 
in the GOM. Gag live to be over 30 years old and 
...
Gag spawn at the shelf edge from February to mid-April in lek-like reproductive units and produce pelagic eggs that settle 
...
nearshore structure after the spawning season. All Gag start life as females, and
...
Additionally, it has been shown that skipped spawning is relatively common in female Gag,
and this is thought to be linked to low male abundance [@lowerre2022spawning]. 
It is unknown to what extent sperm limitation due to reduced male recruitment 
...
misspecifying both $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\Hmt$ in the estimation model (Section 2.6.1).
...
had the same probability of reproducing. All males were considered mature, and $\mathrm{RV}_{a,M}$ 
...
vulnerability of age-$a$ fish to fleet $f$. In the real-world Gag fishery, there are
multiple recreational and commercial hook-and-line fleets, where the recreational
fleets tend to select for smaller individuals and the commercial fleets for larger
fish. We simulated a single recreational and a single commercial fishery for simplicity.
...
(such as the probability of a female of age $a$ reproducing). The event (e.g., reproduction) 
was considered to occur if the random draw fell below that probability.
...
ending with a highly depleted population. For each OM configuration (Section 2.4.1), 40
independent replicates of the population were simulated, providing 40
unique whole-population pedigrees.

### 2.4.1. OM Configurations

The true $\mathrm{RV}_{a,M}$ in protogynous hermaphrodites, including Gag,
is unknown. For Gag, it is assumed that sperm limitation exists but it has not been
measured. We simulated four alternative versions of 'reality' in our OM to explore the
effect of uncertainty about male contribution to reproductive success. In
the base case, declining male abundance had no effect (meaning all eggs produced
by females will get fertilized, regardless of sex ratio) and the male and female (age-specific) 
reproductive value functions ($\mathrm{RV}_{a,M}^\mathrm{OM}$ and $\mathrm{RV}_{a,F}^\mathrm{OM}$, 
respectively) were equal. This corresponds to current Gag stock assessment model assumptions.

In the second configuration, we simulated the effect of sperm limitation 
by increasing the likelihood of female skipped spawning as the male sex ratio
...
current proportion of males in a breeding group relative to unfished levels, $p_M$,
and the probability of females in that breeding group skipping spawning was
modeled as an exponential function of the form $1-\exp(-{\tau p_M})$ (Figure 2B), 
where $\tau$ controls the slope of the curve. We used a sigmoid
...
In the third and fourth OM configurations, we allowed $\mathrm{RV}_{a,M}^\mathrm{OM}$ to differ 
from $\mathrm{RV}_{a,F}^\mathrm{OM}$---either much flatter (nearly age-invariant) or steeper (with older 
individuals contributing substantially more than younger ones; Figure 2A).
...
These served as data inputs for the estimation model (Section 2.6). 

Samples for CKMR were drawn
during the final years of the simulation, from all dead individuals in the population, 
sampling age 1 for juveniles and ages 4+ for adults.
The kin relationship between every pair of sampled individuals was identified from the pedigree, 
and mtDNA identity by descent was determined based on whether the parent or the shared parent 
was the mother or the father (i.e., from the sex of the parent in POPs and the shared parent in HSPs).
...
all sampling years (Section 2.5.1) was chosen as various multiples of the
...
that sampling effort was appropriate to the simulated abundance. We used the average simulated abundance of
...
The simulated traditional fisheries data consisted of a relative recruitment index and age-composition data, 
...
### 2.5.1. Genetic Sampling Scenarios
...
heuristic. The EM was additionally run with 50% and 150% of that base sample size. To limit the 
complexity of the present study, we did not include any further sampling scenarios but note the 
importance of sampling assumptions on simulation results [see, e.g., @conn2020robustness]. 
...
($\mathrm{RV}_F^\mathrm{OM} = \mathrm{RV}_F^\mathrm{EM}$, more on this in Section 2.6.1). Additional estimated parameters were mean
...
### 2.6.1. EM Configurations

EM configurations focused on the estimability and/ or misspecification of key reproductive and 
genetic parameters. In the base EM configuration,
the degree of mitochondrial haplotype diversity in the population ($\Hmt$) was assumed known. 
In two alternative configurations, $\Hmt$ was misspecified (fixed either higher 
or lower than what was specified in the OM). $\Hmt$ affects our ability
to correctly infer the sex of an unobserved parent, or of an observed parent at a time
prior to sampling. It must be estimated 
outside the integrated assessment model, and we tested the effect of 
getting it wrong. $\Hmt$ is often used in HSP calculations because the sex of the 
shared parent is not observed.  In sex-changing species, the sex of the parent at the offspring's time 
...
equifrequent haplotypes). To illustrate the importance of correctly specifying $\Hmt$, we focus
here on HSPs. Given that HSPs sharing a
mother are going to be much more rare than HSPs sharing a father under extreme sex ratio skew toward females,
...
All estimation was performed under four options 
of treating $\mathrm{RV}_{a,M}$ and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ as fixed or estimated. Either both quantities 
where fixed, both were estimated, or one was fixed and one estimated. This was done to explore the 
...
was fixed, it was always fixed at the value corresponding to female reproductive value ($\mathrm{RV}_{a,F}$), mimicking real world stock assessment assumptions.
Consequently, $\mathrm{RV}_{a,M}$ was misspecified in the EM for the OM scenarios where it 
differed from that of the females, allowing 
...
We illustrate the expected effect of misspecifying $\mathrm{RV}_{a,M}$ for the case where 
...
From the above-described OM, sampling model, and EM configurations, we summarize the following scenarios. In the base case, the
OM set $\mathrm{RV}_{a,M}$ equal to $\mathrm{RV}_{a,F}$ and did not simulate sperm limitation; the sampling model collected 
$10\sqrt{N}$ samples over three years; and the EM fixed $\Hmt$, $\mathrm{RV}_{a,M}$, and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ at the 
true values (Figure 3). To explore the effect of CKMR data information content and structure, we (1) extended the 
sampling timeframe to 10 years and varied CKMR sample sizes to 50% and 150% of the base case, and 
(2) evaluated the effect of genetic 
marker resolution by misspecifying $\Hmt$ in two scenarios ($\Hmte$ greater and smaller than $\Hmts$). To evaluate uncertainty in 
male contribution to reproductive success, we (3) simulated skipped spawning in the OM and (4) 
simulated $\mathrm{RV}_{a,M}^\mathrm{OM}$ differing 
from $\mathrm{RV}_{a,F}^\mathrm{OM}$ in two scenarios---with greater and lower contrast in 
reproductive contribution between younger and older 
males than for the females. Finally, to examine whether $\mathrm{RV}_{a,M}$ and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ could be estimated, and to assess 
the effect of misspecifying $\mathrm{RV}_{a,M}$, we ran each scenario fixing or estimating these quantities. 
When fixed, $\mathrm{RV}_{a,M}$ was always fixed at the same values as $\mathrm{RV}_{a,F}$, which resulted in misspecification for (4).
...
the median PRE was calculated across years, and these medians were used to calculate median ($\mathrm{PRE_{sc}}$) and interquartile
range ($\mathrm{IQR_{sc}}$) to compare accuracy and precision of estimates, respectively, by tested scenario (sc). 
...
sperm limitation in the final 35-40 years of the simulation (Figure 4).

## 3.2. Simulated CKMR Samples
The number of females sampled was about two magnitudes higher than the number of
males sampled (Table S1, Figure S3). Additionally, slightly fewer overall 
female CKMR samples were collected when sampling was spread out over the longer period.
...
individuals ($\mathrm{IQR_{sc}} = 3$) when CKMR sample size was 50% of $n_\mathrm{base}$ and sampling was
conducted over three years to 50 individuals ($\mathrm{IQR_{sc}} = 10$) when CKMR sample size was
150% of $n_\mathrm{base}$ and samples were collected over ten years.
For adult females, the number sampled ranged from a median of 1480 ($\mathrm{IQR_{sc}} = 100$) for
50% of $n_\mathrm{base}$ and ten sampling years to a median of 4664 ($\mathrm{IQR_{sc}} = 497$) for
150% of $n_\mathrm{base}$ and three sampling years (Table S1; Figure S3).

The number of CKMR pairs found was higher when sampling was conducted
over a shorter time period (Table S1, Figure S4). Additionally, the number
...
from 166 ($\mathrm{IQR_{sc}} = 52$) for low CKMR sample size to to 1,481 ($\mathrm{IQR_{sc}} = 440$) for
high CKMR sample size (when averaged across number of sampling years),
compared to a median of 10 ($\mathrm{IQR_{sc}} = 6$) to 83 ($\mathrm{IQR_{sc}} = 34$) pairs for
...
(Figure 5). First, the number of unrelated pairs was much
...
The estimation model was fitted to 57,600 different data sets: 40 pedigrees subsampled ten times each
for seven OM+EM scenarios, four parameter estimation combinations, three CKMR sample sizes, 
and two CKMR sampling intervals \footnote{plus another 6,400 data sets for additional runs we
...
weights, for  2,276  runs (3.5%). Thirty-eight of the failed runs were for the smallest CKMR sample size
runs while only 26% were for the largest sample size. 
An additional 4,519 EM runs (7.1%) were excluded because
parameters were estimated at bounds. Most of these were due
to the $\mathrm{RV}_{a,M}^\mathrm{EM}$ function exponent parameter. Finally, 241 additional runs (0.4%) were excluded
because the maximum parameter gradient exceeded 0.1.
Overall, 89% of the simulated data sets produced model fits
that were used in subsequent analyses. Fourteen percent of runs converged and passed
the data weight checks after the first time fitting the model and an additional
29% after the second time. If the model ran 3 times, we used the
run with the lowest negative log likelihood amongst those 3 runs.

## 3.4. Effect of CKMR data information content and structure on estimation performance
...
### 3.4.1. CKMR sample size and number of sampling years

When $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\mathrm{RV}_{a,M}$ were assumed known, median abundance estimates
had no (median $\mathrm{PRE_{sc}} < \pm 1\%$) or low (median $\mathrm{PRE_{sc}} < \pm 2\%$) bias, with no difference between
male and female estimates. Precision was relatively high, with $\mathrm{IQR_{sc}}$
ranging from 5% to 13% across all tested CKMR sample sizes and numbers of CKMR sampling 
years (Figure 6A, Table S2). There was no difference
in median bias between three and ten years of CKMR sampling and precision
was only slightly lower for ten years of sampling. 
Precision ($\mathrm{IQR_{sc}}$) increased by a relatively small amount (28%) when sample
size was increased from 100% to 150% of $n_\mathrm{base}$ and more significantly
(${\approx}67\%$) when the sample size increased from 50% to 100% of $n_\mathrm{base}$ (Figure 6A).

When $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and/or $\mathrm{RV}_{a,M}$ were free to be estimated, median bias remained low
($\mathrm{PRE_{sc}}$ ranging between -3% to 7% across CKMR sampling scenarios)
but precision decreased ($\mathrm{IQR_{sc}}$ ranging from 5% to 51%) and some individual runs had high error.
$\mathrm{IQR_{sc}}$ was similar between males and females when $\mathrm{RV}_{a,M}$ was estimated
(Figure 6C and 6D) but smaller for males than females
when only $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was estimated (Figure 6B). The number
of sampling years continued having only a minor impact on estimation. Generally, $\mathrm{IQR_{sc}}$ increased when
...
(Figure 6B and 6D). CKMR sample size continued to
impact precision more than the median of the abundance estimates. $\mathrm{IQR_{sc}}$ increased from 29% to 41% when sample
...
### 3.4.2. Uncertainty in mitochondrial DNA haplotype diversity

In the base OM and EM configuration ($\Hmte$ = $\Hmts$, no skipped spawning, 
$\mathrm{RV}_{a,M}^\mathrm{OM}$ = $\mathrm{RV}_{a,F}^\mathrm{OM}$), male and female abundance were estimated very well
(Figure 7A, Table S2). Estimates for both sexes had negligible bias
($\mathrm{PRE_{sc}} < \pm 2\%$) regardless of whether $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\mathrm{RV}_{a,M}$ were estimated or
fixed. Precision, however, was higher when both these functions were assumed
known ($\mathrm{IQR_{sc}} {\approx} 7\%$). Precision was lowest for males when both functions were estimated
($\mathrm{IQR_{sc}} = 33\%$)(Figure 7A). 

Misspecifying $\Hmt$ in the EM had major effects on abundance estimation
performance, but only when $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\mathrm{RV}_{a,M}$ were not both 
assumed known (Figure 7B and 7C).
...
were more nuanced. When only $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was assumed known, male
bias mirrored that of the females (Figure 7B and 7C)
resulting in unbiased sex ratio estimates (Figure 8B and 8C)
despite high bias in absolute abundance estimates. This highlights the near-total
control the sex transition function in the EM has over estimated
sex ratios---when it was assumed known, sex ratios were almost always unbiased and precise (Figure 8). Allowing
$P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ to be estimated when $\Hmt$ was misspecified in the EM resulted in highly
biased sex ratio estimates (Figure 8B and 8C) that became slightly less biased for the $\Hmts < \Hmte$ scenario when $\mathrm{RV}_{a,M}$ was also allowed to be estimated (Figure 8B).
...
data (Figure 7B). It does this by underestimating mean recruitment (Figure S6)
along with changing the shape of $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\mathrm{RV}_{a,M}^\mathrm{EM}$,
if allowed (Figure 9D).
When $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ is fixed, thereby locking in the sex ratio, the EM can only reduce the expected
number of females by also lowering the expected number of males (Figure 7B), and it does this by
flattening the estimated $\mathrm{RV}_{a,M}$ function (Figure 9D). This also reduces
...
results in females turning into males at a younger age, reducing the bias in $\mathrm{RV}_{a,M}^\mathrm{EM}$
required to reconcile observed and expected numbers
(Figure 9D). The opposite is true when the EM sees fewer
...
more expected females (Figure 9C).

## 3.5. Effect of Uncertainty about Male Contribution to Reproductive Success
...
chance of encountering $\HSPS$ increases against the EM's expectation and female abundance is underestimated (Figure 7D).
Under a known $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$, the negative bias for both sexes was
low ($\mathrm{PRE_{sc}} < 5\%$) when $\mathrm{RV}_{a,M}$ was also known but increased
when $\mathrm{RV}_{a,M}$ was estimated ($\mathrm{PRE_{sc}} <20\%$; Figure 7D).
When $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was estimated, it was estimated to be slightly less steep than simulated
(Figure 9B), and the resulting female negative bias increased to 
${\approx} 28\% \mathrm{PRE_{sc}}$ but male bias decreased ($\mathrm{PRE_{sc}} <5\%$; Figure 7D).
...
(known $\mathrm{RV}_{a,M}^\mathrm{EM}$: $\mathrm{PRE_{sc}} = 38\%$, $\mathrm{IQR_{sc}} = 30\%$;
estimated $\mathrm{RV}_{a,M}^\mathrm{EM}$: $\mathrm{PRE_{sc}} = 34\%$, $\mathrm{IQR_{sc}} = 49\%$;
Figure 8D; Table S2).

### 3.5.2. Differing simulated reproductive value between the sexes
When the EM assumed that $\mathrm{RV}_{a,M}$ equaled $\mathrm{RV}_{a,F}$ but the OM simulated them to be different, bias in male abundance
estimates was as predicted: positive when simulated $\mathrm{RV}_{a,M}^\mathrm{OM}$ was flatter and negative when
it was steeper than $\mathrm{RV}_{a,F}^\mathrm{OM}$ (Figure 7E and 7F). Female bias
matched that of the males when $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was known, but when $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was allowed to be
estimated, median female abundance estimates were fairly unbiased, driven by compensation
for any erroneous $\mathrm{RV}_{a,M}^\mathrm{EM}$ in the estimated shape of $P_a^{\mathrm{F}\rightarrow\mathrm{M}}$
(Figures 9E&F and S7). When $\mathrm{RV}_{a,M}^\mathrm{EM}$ was
allowed to be estimated, male abundance estimates became much less biased:
for the scenario where $\mathrm{RV}_{a,M}^\mathrm{OM}$ was flatter than $\mathrm{RV}_{a,F}^\mathrm{OM}$,
$\mathrm{PRE_{sc}}$ was reduced to ${\approx}{5}\%$ from ${\approx}{55}\%$, and for the
scenario where $\mathrm{RV}_{a,M}^\mathrm{OM}$ was steeper than $\mathrm{RV}_{a,F}^\mathrm{OM}$,
$\mathrm{PRE_{sc}}$ was reduced to ${\approx}{-6}\%$ from ${\approx}{-64}\%$ (Figure 7E and 7F).
Sex ratios were most biased when
$P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was allowed to be estimated but $\mathrm{RV}_{a,M}$ continued to be
misspecified ($\mathrm{PRE_{sc}} = 51\%$ and $\mathrm{IQR_{sc}} = 37\%$ for $\mathrm{RV}_{a,M}^\mathrm{OM}$ flatter than 
$\mathrm{RV}_{a,F}^\mathrm{OM}$;
$\mathrm{PRE_{sc}} = -65\%$ and $\mathrm{IQR_{sc}} = 11\%$ for $\mathrm{RV}_{a,M}^\mathrm{OM}$ steeper than 
$\mathrm{RV}_{a,F}^\mathrm{OM}$; Figure 8E and 8F).

## 3.6. Ability to estimate sex transition and male reproductive value
The sex transition function shape was overall well estimated with little variability in
estimated shapes, except when $\mathrm{RV}_{a,M}^\mathrm{OM}$ was steeper than 
$\mathrm{RV}_{a,F}^\mathrm{OM}$ and $\mathrm{RV}_{a,M}$ was also estimated (Figure 9F). 
The $\mathrm{RV}_{a,M}$ function was much more variable (Figures 9 and S7).
The median was estimated fairly well except when $\Hmt$ was misspecified in the EM 
but $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ remained fixed (Figure 9C&D)
and when $\mathrm{RV}_{a,M}^\mathrm{OM}$ was flatter than $\mathrm{RV}_{a,F}^\mathrm{OM}$ 
(Figure 9E). Generally, the median estimated $\mathrm{RV}_{a,M}$ shapes matched the simulated curvatures,
...
became flatter, $\mathrm{RV}_{a,M}^\mathrm{EM}$ became steeper (Figure S8). This correlation increased
with increasing CKMR sampling in all scenarios except when $\mathrm{RV}_{a,M}^\mathrm{OM}$
was close to age-invariant.
$\mathrm{STF}_{SD}$ was moderately negatively correlated with mean recruitment
but this correlation decreased with increasing CKMR sampling (Figure S8).
...
females.  When the EM falsely assumed $\mathrm{RV}_{a,M} = \mathrm{RV}_{a,F}$, this resulted in significant estimation bias, but
we were encouraged to see that there was enough information in the data to allow $\mathrm{RV}_{a,M}$ to be estimated with no or little bias
...
go about attempting to measure $\mathrm{RV}_{a,M}$ in the Gag population. Theoretically, one could obtain 
it by dividing the sample age composition of adult males by the sample age composition of 
male parents which allows separating $\mathrm{RV}$ from selectivity-at-age [@davies2017advice], but this 
would be extremely difficult, in practice, for Gag due to the expectation of low numbers 
...
to be important to Gag population dynamics. The most important of these is likely spatial effects. 
...
[@ingram2021juvenile]. Additionally, composition data are highly uncertain and particularly sparse for the private
...
thank two anonymous reviewers for their helpful suggestions and constructive
...
an age-$a$ male ($b$ is just a summation index).
