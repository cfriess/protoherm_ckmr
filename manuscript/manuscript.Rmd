...
strategies—varying both the number of years and the sample size—to provide both traditional fisheries data and
...
we used to test the performance of our CKMR estimators (Figure 1).
...
the base OM its exponent was set equal to that of $\mathrm{RV}_F$ (Figure 3, panel A), 
...
ending with a highly depleted population. For each OM scenario (Section 2.3.1), 40
...
from $\mathrm{RV}_F^\mathrm{OM}$ (Figure 3, panel A).
...
3, panel B), where $\tau$ controls the slope of the curve. We used a sigmoid
...
These served as data inputs for the estimation model (Section 2.5). 
...
The total number of CKMR samples collected across all sampling years (Section 2.4.1) was chosen as various multiples of the
...
### 2.4.1. Genetic Sampling Scenarios
...
contained misspecification (Figure 1). 
...
## 3.4. Effect of CKMR Information Availability on Estimation Performance
...
years (Figure 6). There was no difference
...
(Figure 6, panels C and D) but smaller for males than females
when only $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was estimated (Figure 6, panel B). The number
...
(Figure 6, panels B and D). CKMR sample size continued to
...
### 3.4.2. Types of CKMR pairs used in the likelihood

Male and female abundance were estimated very well in the base OM and EM configuration.
(Figure 7, panel A). Estimates for both sexes had negligible bias
...
($\mathrm{IQR_{PRE}} = 33\%$)(Figure 7, panel A). There was no difference in
...
or estimated in the model (Figure 7, panels A and B).
...
### 3.4.3. Uncertainty in mitochondrial DNA haplotype diversity
...
bias mirrored that of the females (Figure 7, panels C & D)
resulting in unbiased sex ratio estimates (Figure 8, panels C & D)
...
allowed to be estimated (Figure 8, panels C & D).
...
flattening the estimated $\mathrm{RV}_M$ function (Figure 9). This also reduces
...
(Figure 9). The opposite is true when the EM sees fewer
...
more expected females (Figure 9).
...
resulting in fairly unbiased sex ratio estimates (Figure 8). We, therefore, focus
...
increases against the EM's expectation and female abundance is underestimated (Figure 10, panel B).
...
(Figure 9), the female negative bias increased to a median value of
...
it was steeper than $\mathrm{RV}_F^\mathrm{OM}$ (Figure 10, panels C and D). Female bias
...
(Figures 9 and S8). When $\mathrm{RV}_M^\mathrm{EM}$ was
...
${\approx}{-6}\%$ from ${\approx}{-64}\%$ for steeper $\mathrm{RV}_M^\mathrm{OM}$; Figure 10, panels C and D).
...
$\mathrm{PRE} = -65\%$ and $\mathrm{IQR_{PRE}} = 11\%$ for steeper $\mathrm{RV}_M^\mathrm{OM}$; Figure 8, panels E and F).
...
estimated shapes. The $\mathrm{RV}_M$ function was much more variable (Figures 9 and S8).
...
but this correlation decreased with increasing CKMR sampling (Figure 9).
