...
abstract: |
  ...
  species, however, are sequential hermaphrodites for which preferential harvest for certain size classes can drastically 
...
in the CKMR likelihood. Gag is of particular interest because its limited plasticity in sex 
change has led to extreme male scarcity under heavy fishing pressure. This has lead to concerns 
...
spawn in pairs, unlike other aggregating groupers that broadcast release their gamtes. 
...
individuals contributing substantially more than younger ones; Figure 3A). We note again here 
that the specific OM configurations explored in simulation should be informed by the biology 
of the model species and the specific research questions at hand. 
...
and mtDNA identity by descent was determined based on whether the shared parent was the mother 
or the father (i.e., from the sex of the parent in POPs and the shared parent in HSPs).
...
from traditional fisheries data uncertainty. We note that fully operational simulation studies would typically 
include stochasticity in all sampled data.

Catch data were not generated because these data are often highly uncertain for protogynous 
...
importance of sampling assumptions on simulation results [see, e.g., @conn2020robustness]. If the OM 
includes detailed spatial dynamics, the sampling model might explore more fully the effects
of spatio-temporal sampling variability and age-specific sampling selectivity on estimation 
performance.
...
and age-specific male reproductive value (male RV), were fixed at the true value (i.e., assumed known). 
...
getting it wrong. This quantity affects both the POP and
HSP calculations because it represents the chance of incorrectly assigning
related pairs (Equations A8 and A14-A16, Supplemental Information on assumption on 
...
of treating male RV and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ as fixed or estimated. Either both quantities 
...
Importantly, when male RV 
was fixed, it was always fixed at the value corresponding to female RV, mimicking real world stock assessment assumptions.
Consequently, male RV was misspecified in the EM for the OM scenarios where it differed from that of the females, allowing 
us to explore the consequences of getting male RV wrong in the assessment. 
...
We illustrate the expected effect of misspecifying male RV for the case where $\mathrm{RV}_M^\mathrm{OM}$ is steeper than $\mathrm{RV}_F^\mathrm{OM}$. Here, younger males in the simulated population contribute fewer offspring and the older 
...
OM set male RV equal to that of females and did not simulate sperm limitation; the sampling model collected 
$10\sqrt{N}$ samples over three years; and the EM fixed $\Hmt$, male RV, and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ at the 
...
male contribution to reproductive success, we (3) simulated skipped spawning in the OM and (4) simulated male RV differing 
...
ages than for the females. Finally, to examine whether male RV and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ could be estimated, and to assess 
the effect of misspecifying male RV, we ran each scenario fixing or estimating these quantities. 
When fixed, male RV was always fixed at the same values as female RV, which resulted in misspecification for (4).
...
Other protogynous hermaphrodites have shown more plasticity in sex change (REFS?), where females start 
