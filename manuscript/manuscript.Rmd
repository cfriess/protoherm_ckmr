...
There were, however, a number of iterations (only for the males and primarily within two cases; Figure S3) 
...
The number of females sampled in the base case was about two orders of magnitude higher 
...
the number of HSPs sharing mtDNA ($\HSPS$). Additionally, POPs with matching mtDNA were 
seven times more common than those with different mtDNA (36 vs 5; Table S1, Figure S7). 
...
three-fold compared to the base case (Figure 4, Table S1).
...
## 3.4. Base Case Estimation Performance 
...
resulting in accurate sex ratio estimates (Figure 6). We, therefore, focus
between-sex comparisons in this and the next sections on the case when 
$P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was allowed to be estimated.
...
91 to 254% (Table S2). Precision in female abundance estimates for the same EM configurations 
...
inaccurate in the opposite direction compared to females (Figure 5B&C).
...
(Figure 7D). The opposite is true when the EM sees fewer
...
low ($|\mathrm{PRE_{c}}| < 5\%$) when $\mathrm{RV}_{a,M}$ was also known but increased
...
Precision in male and female estimation was variable between scenarios, as low as 
...
to within $\pm{20}\%$ (Figure 6E&F), even though precision in sex ratio estimates was 
...
However, in almost all scenarios, some runs estimated incorrect curvatures (i.e., estimated
...
is larger than the true number of maternal HSPs sharing a mother but the EM won't know this, 
...
but the EM discounts the true number of maternal HSPs because it 
assumes some of them are HSPs with different mitochondrial DNA ($\HSPD$), and it overestimates female
abundance.
...
fewer) contribute more to reproduction. Consequently, more HSPs sharing a father should be found than what the EM expects. 
This would signal to the EM that there are
fewer males in the population and male abundance should be underestimated. When $\mathrm{RV}_{a,M}^\mathrm{OM}$
is flatter (i.e., more age-invariant) than $\mathrm{RV}_{a,F}^\mathrm{OM}$ 
(but $\mathrm{RV}_{a,F}^\mathrm{EM}$ = $\mathrm{RV}_{a,M}^\mathrm{EM}$), the opposite would be the case: younger males would
...
Additionally, neither our OM nor our EM modeled size variation at age.
In real populations, there is variation in size at age, which matters because many
