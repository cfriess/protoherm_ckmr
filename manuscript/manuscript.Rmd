...
abstract: |
  ...
  species, however, are protogynous hermaphrodites for which preferential harvest for certain size classes can drastically 
...
with an alpha male typically exhibit rapid sex change which can mitigate fishing-induced male depletion [@warner1982mating]. In contrast, species that form leks or spawning aggregations may be more vulnerable because their sex-change dynamics and response 
to male removal remain poorly understood, and some evidence from exploited stocks suggests frequent male 
depletion under fishing pressure [@lowerre2020testing; @alonzo2004effects; @alonzo2005sex].
...
obscure or invalidate results. While the paper focuses on Gag, the CKMR estimators and general 
simulation framework are adaptable to a wide range of protogynous hermaphrodites.

# 2. Methods

## 2.1. Overview

We start out this section by describing the CKMR kinship probabilities for protogynous hermaphrodites (Section 2.2).
...
of protogynous hermaphrodites, modeled after the biology of Gag (Section 2.3). Each simulation creates millions of
...
misspecifying both $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\Hmt$ in the estimation model (Section 2.5.1).
...
Gag spawn at the shelf edge from February to mid-April. While being polygamous, gag likely 
spawn in pairs, unlike other aggregating groupers that broadcast release their gametes. 
...
[@lowerre2020testing; @SEDAR2022]. Additionally, it has been shown that skipped spawning is relatively common in female gag,
and this is thought to be linked to low male abundance [@lowerre2022spawning].

Gag make an excellent case study for sperm limitation, as their spatial ecology (offshore spawning estuarine nursery grounds), sexual segregation, and lek-like reproductive unit results in  decreased male recruitment with little change in the size and age at transition (Lowerre-Barbieri et al. 2020).


It is unknown to what extent sperm limitation 
...
of the year, followed by mortality, CKMR sampling, and age incrementation (Figure 2).
...
had the same probability of reproducing. All males were considered mature, and $\mathrm{RV}_M$ 
...
the base OM its exponent was set equal to that of $\mathrm{RV}_F$ (Figure 3A), 
...
here that this is not typical for protognous hermaphrodites. Simulation studies modeled on species 
that exhibit phenotypic plasticity in sex change should allow females to transition at younger ages 
...
reproductive value functions ($\mathrm{RV}_M^\mathrm{EM}$ and $\mathrm{RV}_M^\mathrm{OM}$, 
...
3B), where $\tau$ controls the slope of the curve. We used a sigmoid
...
In the third and fourth OM configurations, we allowed male reproductive value (RV) to differ 
from that of females—--either much flatter (nearly age-invariant) or steeper (with older 
individuals contributing substantially more than younger ones; Figure 3A).
...
but all samples were used for both HSPs and POPs.
The total number of CKMR samples collected across all sampling years (Section 2.5.1) was chosen as various multiples of the
...
from traditional fisheries data uncertainty. 

Catch data were not generated because the absolute scale of landings and discards is highly uncertain for protogynous 
...
All life history and fishery vulnerability parameters except sex change probability, $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$, 
and age-specific male reproductive value ($\mathrm{RV}_M$), were fixed at the true value (i.e., assumed known). 
...
($\mathrm{RV}_F^\mathrm{OM} = \mathrm{RV}_F^\mathrm{EM}$, more on this in Section 2.5.1). Additional estimated parameters were mean
...
Mitochondrial haplotype diversity ($\Hmt$) must be estimated 
outside the integrated assessment model, and we tested the effect of 
getting it wrong. This quantity is often used in HSP calculations because the sex of the 
...
of treating $\mathrm{RV}_M$ and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ as fixed or estimated. Either both quantities 
...
types of misspecification (skipped spawning, wrong $\mathrm{RV}_M$, wrong $\Hmt$).
Importantly, when $\mathrm{RV}_M$ 
was fixed, it was always fixed at the value corresponding to female reproductive value ($\mathrm{RV}_F$), mimicking real world stock assessment assumptions.
Consequently, $\mathrm{RV}_M$ was misspecified in the EM for the OM scenarios where it differed from that of the females, allowing 
us to explore the consequences of getting $\mathrm{RV}_M$ wrong in the assessment. 
When $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was fixed, on the other hand, it was always fixed at the true value. Allowing 
the model to attempt to estimate $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ was done to determine if the CKMR data 
could allow estimating the sex ratio.

We illustrate the expected effect of misspecifying $\mathrm{RV}_M$ for the case where $\mathrm{RV}_M^\mathrm{OM}$ is steeper than $\mathrm{RV}_F^\mathrm{OM}$. Here, younger males in the simulated population contribute fewer offspring and the older 
males (of which there are
...
OM set $\mathrm{RV}_M$ equal to that of females and did not simulate sperm limitation; the sampling model collected 
$10\sqrt{N}$ samples over three years; and the EM fixed $\Hmt$, $\mathrm{RV}_M$, and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ at the 
true values (Figure 1). To explore the effect of CKMR data information content and structure, we (1) extended the 
...
male contribution to reproductive success, we (3) simulated skipped spawning in the OM and (4) simulated $\mathrm{RV}_M$ differing 
from that of females in two scenarios---with greater and lower contrast in reproductive contribution between younger and older 
ages than for the females. Finally, to examine whether $\mathrm{RV}_M$ and $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ could be estimated, and to assess 
the effect of misspecifying $\mathrm{RV}_M$, we ran each scenario fixing or estimating these quantities. 
When fixed, $\mathrm{RV}_M$ was always fixed at the same values as $\mathrm{RV}_F$, which resulted in misspecification for (4).
...
range ($\mathrm{IQR_{sc}}$) of $\mathrm{PRE_{sc}}$ to compare accuracy and precision of estimates, respectively, by tested scenario (sc). 
...
to the $\mathrm{RV}_M^\mathrm{EM}$ function exponent parameter. Finally, 241 additional runs (0.4%) were excluded
...
years (Figure 6A, Table S1). There was no difference
...
$\mathrm{IQR_{sc}}$ was similar between males and females when $\mathrm{RV}_M$ was estimated
...
sampling was spread out over ten years (an exception was male estimation performance when both $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\mathrm{RV}_M$ were estimated). The largest increase (by ${\approx}11\%$) was observed for
...
$\mathrm{RV}_M^\mathrm{OM}$ = $\mathrm{RV}_F^\mathrm{OM}$), male and female abundance were estimated very well
(Figure 7A, Table S1). Estimates for both sexes had negligible bias
($\mathrm{PRE_{sc}} < \pm 2\%$) regardless of whether $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\mathrm{RV}_M$ were estimated or
...
performance, but only when $P_a^{\mathrm{F}\rightarrow \mathrm{M}}$ and $\mathrm{RV}_M$ were not both 
...
biased sex ratio estimates (Figure 8B and 8C) that became slightly less biased for the $\Hmts < \Hmte$ scenario when $\mathrm{RV}_M$ was also allowed to be estimated (Figure 8B).
...
flattening the estimated $\mathrm{RV}_M$ function (Figure 9D). This also reduces
...
contributing offspring, effectively shifting $\mathrm{RV}_F^\mathrm{OM}$ to the right, towards older ages. 
...
low ($\mathrm{PRE_{sc}} < 5\%$) when $\mathrm{RV}_M$ was also known but increased
when $\mathrm{RV}_M$ was estimated ($\mathrm{PRE_{sc}} <20\%$; Figure 7D).
...
(known $\mathrm{RV}_M^\mathrm{EM}$: $\mathrm{PRE_{sc}} = 38\%$, $\mathrm{IQR_{sc}} = 30\%$;
estimated $\mathrm{RV}_M^\mathrm{EM}$: $\mathrm{PRE_{sc}} = 34\%$, $\mathrm{IQR_{sc}} = 49\%$;
Figure 8D; Table S1).

### 3.5.2. Differing simulated reproductive value between the sexes
When the EM assumed that $\mathrm{RV}_M$ equalled $\mathrm{RV}_F$ but the OM simulated them to be different, bias in male abundance
estimates was as predicted: positive when simulated $\mathrm{RV}_M^\mathrm{OM}$ was flatter and negative when
it was steeper than $\mathrm{RV}_F^\mathrm{OM}$ (Figure 7E and 7F). Female bias
...
(Figures 9E&F and S7). When $\mathrm{RV}_M^\mathrm{EM}$ was
allowed to be estimated, male abundance estimates became much less biased:
for the scenario where $\mathrm{RV}_M^\mathrm{OM}$ flatter than $\mathrm{RV}_F^\mathrm{OM}$,
$\mathrm{PRE_{sc}}$ was reduced to ${\approx}{5}\%$ from ${\approx}{55}\%$, and for the
scenario wehre $\mathrm{RV}_M^\mathrm{OM}$ steeper than $\mathrm{RV}_F^\mathrm{OM}$,
...
misspecified ($\mathrm{PRE{sc}} = 51\%$ and $\mathrm{IQR_{sc}} = 37\%$ for $\mathrm{RV}_M^\mathrm{OM}$ flatter than 
$\mathrm{RV}_F^\mathrm{OM}$;
$\mathrm{PRE{sc}} = -65\%$ and $\mathrm{IQR_{sc}} = 11\%$ for $\mathrm{RV}_M^\mathrm{OM}$ steeper than 
$\mathrm{RV}_F^\mathrm{OM}$; Figure 8E and 8F).
...
estimated shapes, except when $\mathrm{RV}_M^\mathrm{OM}$ was steeper than 
$\mathrm{RV}_F^\mathrm{OM}$ and $\mathrm{RV}_M$ was also estimated (Figure 9F). 
The $\mathrm{RV}_M$ function was much more variable (Figures 9 and S7).
...
and when $\mathrm{RV}_M^\mathrm{OM}$ was flatter than $\mathrm{RV}_F^\mathrm{OM}$ and $\mathrm{RV}_M$ 
(Figure 9E). Generally, the median estimated $\mathrm{RV}_M$ shapes matched the simulated curvatures,
...
was close to age-invariant (Figure S8).
...
go about attempting to measure $\mathrm{RV}_{M}$ in the gag population. Theoretically, one could obtain 
...
to be important to gag population dynamics. The most crucial of these are probably spatial effects. 
