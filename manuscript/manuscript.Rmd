...
We chose the median over the mean here after inspecting the PRE values within case (Figure Sx). 
For most cases, PREs were flat over years for all iterations, so median and mean perform similarly. 
There were, however, a number of iterations (primarily within two cases) where extreme PRE values early in
the time series distort the mean while the median captures the dominant signal (see Supplemental Material).
...
cases, but some exhibited right-skew and extreme tails (Figure Sx) and the median better describes
...
than the number of males sampled (median of 3,110 females vs 28 males; Table S1, Figure S3).
The number
...
seven times more common than those with different mtDNA (36 vs 5; Table S1, Figure S4). 
...
compared to the base case (Figure 4).
...
inaccurate in the opposite direction compared to females (Figures 5B&C).
...
(Figure S5). Thus when the EM sees more $\HSPS$ than it expects
in the case of $\Hmts$ < $\Hmte$,
it attempts to reduce the number of expected females to match what it sees in the genetic
data (Figure 5B). It does this by underestimating mean recruitment (Figure S6)
...
overestimated (Figure S6) and the estimated
...
The $\mathrm{RV}_{a,M}$ function was much more variable (Figures 7 and S7).
...
became flatter, $\mathrm{RV}_{a,M}^\mathrm{EM}$ became steeper (Figure S8). 
$\mathrm{STF}_{SD}$ was moderately negatively correlated with mean recruitment (Figure S8).
